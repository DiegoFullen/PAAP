...
def update_hours(request):
    try:
        ...
        if not hours:
            print("Horas no proporcionadas en la URL.")
            return redirect('payment')

        hours = int(hours)  # Asegurarse de que sea un entero

        email = request.session.get('email')
        if not email:
            print("Email no encontrado en la sesión.")
            return redirect('login')

        with connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT * FROM gestion_usuarios_plan WHERE email_id = %s
                """,
                [email]
            )
            result = cursor.fetchone()
            if not result:
                print("No se encontró un registro para el email.")
                return redirect('payment')

            horas = result[1]  # Validar que la columna de horas esté en esta posición
            f = hours + horas
        ...
        print("Horas proporcionadas no son válidas.")
        return redirect('payment')
    except Exception as e:
        print(f"Error actualizando horas: {e}")
        return redirect('payment')
